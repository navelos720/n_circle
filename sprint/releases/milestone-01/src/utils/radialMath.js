 ...
 * Returns -1 if in dead zone or outside maximum radius.
 ...
 * @param {number} deadZone - Dead zone radius in px
 * @param {number} maxRadius - Outer radius boundary in px
 * @returns {number} slot index (0-based) or -1
 */
export function getSlotIndexFromPoint(x, y, cx, cy, slotCount = 8, deadZone = 40, maxRadius = 150) {
    const dx = x - cx;
    const dy = y - cy;
    ...
    // Wedges are centered on their slot angles or offset:
    // Slot 0 is centered around 0 deg (or from -wedgeAngle/2 to +wedgeAngle/2)
    const halfWedge = wedgeAngle / 2;
    let adjustedAngle = (angle + halfWedge) % 360;
